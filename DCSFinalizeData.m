...
function [vol, hdr, infodcm] = DCSFinalizeData(DCSCurrentData, binaryFormat)
...
for fi = 1:hdr.nframes
	for si = 1:hdr.nplanes
		...
		if fi==1 && si==1
			...
			while isempty(vol1)
				try
					if hdr.nframes>1
						vol = zeros(ceil(hdr.xdim/hdr.DownSampleFactor_xy_z(1)) ,ceil(hdr.ydim/hdr.DownSampleFactor_xy_z(1)) , ceil(hdr.nplanes/hdr.DownSampleFactor_xy_z(2)), hdr.nframes, binaryFormat);
					end
					vol1 = zeros(ceil(hdr.xdim/hdr.DownSampleFactor_xy_z(1)) ,ceil(hdr.ydim/hdr.DownSampleFactor_xy_z(1)) , ceil(hdr.nplanes/hdr.DownSampleFactor_xy_z(2)), binaryFormat);
				catch
	...
	if isnan(hdr.PETTotalCounts(fi)) && isfield(info,'Private_0009_10a7')
		if length(info.Private_0009_10a7)==1
			hdr.PETTotalCounts(fi) = info.Private_0009_10a7;
		else
			DCSlogmsg(' - * Warning! PETTotalCounts field (0009,10A7) not a valid value.');
		end
	end
	...
	switch lower(binaryFormat)
		...
		case {'single','double'}
			vol(:,:,:,fi) = cast(vol1, binaryFormat);
			hdr.quant_dynamic(fi) = 1;
		otherwise
			error('UnknownBinaryFormat:SaveTarget:DicomConertServer','Unrecognized binary format encountered')
	end
end % frame loop
DCSlogmsg('Done',1);
