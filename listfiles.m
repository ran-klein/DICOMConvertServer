% ListFiles - lists files that meet a search criteria.
% files=listfiles(strpat, directory, flag)
%
% Usage:
% ======
% files=listfiles(strpat) - list all the file with string pattern, strpat,
% in the current directory. files is a cell of strings with the file names 
% that meet the pattern in the direcotry. 
...
if any(lower(flag)=='s')
	subdirs = dir([directory]); % removed filesep '*.']) to support directory names with . in them;
