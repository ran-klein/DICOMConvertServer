...
if strcmpi(flag,'start') || strcmpi(flag,'refresh')
	...
	DCSlogmsg(['Binary volume format: ' DCSVars.binaryFormat]);
	if DCSVars.cleanup
	...
	if ~serverStopRequested(DCSVars) && ~isempty(files)
		...
		for i=1:length(files)
			...
			if ~strcmpi(files{i},'.') && ~strcmpi(files{i},'..')
				tempFile = [DCSVars.tempdir filesep files{i}];
				t = load(tempFile,'chklist','dirCheckCounter');
				if t.dirCheckCounter == DCSVars.dirCheckCounter % counter elapsed - deal with this file
					...
					if ~isfield(t.chklist,'table')
						DCSlogmsg(['Rejecting ' files{i} ' due to missing files table.']);
					else
						t.chklist = fixIncorrectMultiframeSeries(t.chklist, tempFile);
						if all(all(t.chklist.table(:,:,1))) % the file is complete
...
% update the filename in the list and move to temporary directory
if valid
...
%% fix a series that was interpreted as multi-frame, but is not.
function chklist = fixIncorrectMultiframeSeries(chklist, tempFile)
numSlices = length(chklist.slice.vals);
numFrames = length(chklist.frame.vals);
numElements = sum(reshape(chklist.table(:,:,2),1,[])>0);
if numElements == numSlices*numFrames
	return; % number of file slices matches header derived number of slices and frames - no modfication required
else
	% There is a mismatch between header-indicated and file-indicated
	% number of total slices
	if numElements == numSlices % Falsely interpreted as geted/dynamic study
		chklist.frame.vals = chklist.frame.vals(1);
		chklist.table = sum(chklist.table,2);
		save(tempFile,'-append','chklist')
	else
		return; % Still not matching, but don't know how to resolve. Leave as is and calling function will reject the series
	end
end
...
[vol, hdr] = DCSFinalizeData(struct('infodcm',infodcm,'chklist',chklist,'hdr',hdr), DCSVars.binaryFormat); %#ok<ASGLU,NODEF>
